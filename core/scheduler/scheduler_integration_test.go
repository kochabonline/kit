...
func TestSchedulerIntegration(t *testing.T) {
	...
	t.Run("完整的调度器生命周期测试", func(t *testing.T) {
		...
		require.NoError(t, err, "等待任务调度超时")

		// 8. 验证任务已被分配给工作节点
		scheduledTask, err := scheduler.GetTask(ctx, task.ID)
		require.NoError(t, err, "获取调度后的任务失败")
		assert.Equal(t, TaskStatusScheduled, scheduledTask.Status)
		assert.Equal(t, worker.ID, scheduledTask.WorkerID)
		assert.True(t, scheduledTask.ScheduledAt > 0)

		// 9. 模拟工作节点执行任务
		err = simulateTaskExecution(ctx, scheduler, scheduledTask)
		require.NoError(t, err, "模拟任务执行失败")
...
func TestSchedulerMultipleTasksExecution(t *testing.T) {
	...
	t.Run("多任务并发执行测试", func(t *testing.T) {
		...
		for _, task := range tasks {
			...
			go func(t *Task) {
				defer wg.Done()
				scheduledTask, err := scheduler.GetTask(ctx, t.ID)
				if err != nil {
					return
				}
				_ = simulateTaskExecution(ctx, scheduler, scheduledTask)
			}(task)
		}
...
func simulateTaskExecution(ctx context.Context, scheduler Scheduler, task *Task) error {
	// 模拟任务开始执行
	task.Status = TaskStatusRunning
	task.StartedAt = currentTimestamp()
	task.UpdatedAt = currentTimestamp()

	err := scheduler.UpdateTask(ctx, task)
	if err != nil {
		return fmt.Errorf("更新任务开始状态失败: %w", err)
	}
...
// simulateTaskFailure 模拟任务失败
func simulateTaskFailure(ctx context.Context, scheduler Scheduler, task *Task, errorMsg string) error {
	task.Status = TaskStatusRunning
	task.StartedAt = currentTimestamp()
	task.UpdatedAt = currentTimestamp()
